...
- **Remaining branch/dependency queue (read-only triage)**: `origin/Tanvir` is merged into `origin/main` and is safe to delete only after explicit user OK. Twelve open Dependabot PRs remain unmerged/unreviewed (#29, #30, #41, #48–#56); recommended order is GitHub Actions majors first, then Next patch train (`next`/`eslint-config-next`/`@next/*`), then isolated patch bumps (`postcss`, `pdfjs-dist`, `read-excel-file`, `@sentry/browser`), and finally major-tooling batches (`@vitejs/plugin-react` 5/6, Vitest 4, ESLint 10, TypeScript 6) with the full gate after each batch.
- **Deferred** (out of scope this round): discriminated-union course state (audit #11), tokens.css contrast bump (visual regression risk requiring screenshot review), Dependabot merges/upgrades, and stale-branch deletion (require explicit user OK per operational-safety policy).
