...
> Last Updated: 2026-04-25 | 272 source files | 26,397 LOC
...
> **Note:** `sw.js` is now generated by `workbox-build` during postbuild into `out/`. The static `public/sw.js` and `public/_headers` were removed. Service worker is registered via `src/shared/components/infra/sw-register.tsx`.

## Source Tree — `src/` (26,397 LOC)

### App Layer — `src/app/` (2,283 LOC)
...
### Features Layer — `src/features/` (13,150 LOC)
...
### Shared Layer — `src/shared/` (10,732 LOC, 41% of codebase)
...
│   │   ├── skip-link.tsx
│   │   ├── structured-data.tsx
│   │   ├── sw-register.tsx          — Service worker registration (Workbox)
...
├── ISSUES.md               — Finding tracker (275 findings, 5 open)
