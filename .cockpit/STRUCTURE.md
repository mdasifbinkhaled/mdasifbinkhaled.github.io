...
```text
scripts/
├── generate-service-worker.mjs — Generates out/sw.js and static precache manifest
├── serve-static.mjs            — Dependency-free static server for Playwright
└── validate-commit-message.mjs — Conventional Commit validator for Husky
```
...
│   │   └── error.tsx       — Error boundary (factory pattern)
│   ├── course-planner/
│   │   ├── page.tsx        — Course dependency planner
│   │   └── error.tsx       — Error boundary (factory pattern)
│   ├── office-hours/
│   │   ├── page.tsx        — Office hours schedule
│   │   └── error.tsx       — Error boundary (factory pattern)
│   ├── pdf-study-aid/
│   │   ├── page.tsx        — Browser-local PDF study aid
│   │   └── error.tsx       — Error boundary (factory pattern)
│   ├── seat-planner/
│   │   ├── page.tsx        — Seat plan generator page
│   │   └── error.tsx       — Error boundary (factory pattern)
│   └── study-timer/
│       ├── page.tsx        — Pomodoro-style study timer
│       └── error.tsx       — Error boundary (factory pattern)
├── blog/
...
│   └── index.ts
│
├── apps/                   — Student apps feature (35 files)
│   ├── index.ts            — Feature barrel export
│   └── components/
│       ├── app-page-header.tsx — Shared app detail header
│       ├── apps-hub.tsx        — Apps hub grid and categories
│       ├── grade-calculator.tsx — Weighted grade calculator
...
│       ├── pdf-study-aid/      — Browser-local PDF study workflow (2 files)
│       │   ├── pdf-study-aid.tsx
│       │   └── study-aid.ts
│       ├── study-timer/        — Pomodoro timer and activity log (1 file)
│       │   └── study-timer.tsx
│       ├── course-planner/     — Course dependency planner (6 files + preset payload)
│       │   ├── course-plan-utils.ts
│       │   ├── course-planner.tsx
│       │   ├── topo-sort.ts
│       │   ├── types.ts
│       │   └── presets/
│       │       ├── iub-cse.ts
│       │       └── index.ts
│       └── seat-planner/       — Seat plan generator (16 files)
│           ├── allocation.ts       — Seat allocation algorithm
│           ├── csv-export.ts       — CSV seat plan export
│           ├── exam-details-form.tsx — Exam details form component
│           ├── export-utils.ts     — PDF/PNG/print export helpers
│           ├── import-utils.ts     — Flexible room/student ingest helpers
│           ├── jspdf-autotable.d.ts — jsPDF autotable type augmentation
│           ├── pdf-export.ts       — PDF seat plan export via jsPDF
│           ├── room-configuration.tsx — Room config UI
│           ├── seat-plan-print-document.tsx — Print-ready seat sheets
│           ├── seat-plan-results.tsx — Results display
│           ├── seat-planner.tsx    — Main orchestration shell
│           ├── shared-ui.tsx       — Shared UI primitives
│           ├── student-data-panel.tsx — Student data input panel
│           ├── types.ts            — Type definitions
│           ├── use-seat-planner.ts — State management hook
│           └── index.ts
│
...
│   ├── utils.ts            — cn() utility
│   └── data/               — Domain data layer (33 files, TypeScript objects)
│       ├── about.ts
│       ├── activities.ts
│       ├── courses.ts      — Course registry + helpers
│       ├── courses/        — Individual course definitions (17 files)
│       │   ├── index.ts
│       │   ├── bracu-cse284.ts, bracu-cse420/, bracu-cse423.ts, bracu-cse489.ts
│       │   ├── iub-cse101.ts, iub-cse110.ts, iub-cse201.ts, iub-cse203.ts
│       │   ├── iub-cse211/ (5 files: index, modules, resources, schedule, term)
...
## Tests — `tests/` (368 unit tests + 49 E2E tests, 46 files)
...
├── e2e/
│   ├── fixtures/
│   │   └── routes.ts           — Shared route definitions for E2E tests
│   ├── all-pages-smoke.spec.ts — Smoke tests for 18 content routes + 3 redirect verifications
│   ├── a11y-audit.spec.ts  — axe-core accessibility audit across 18 routes
│   ├── keyboard-nav.spec.ts — Keyboard navigation and focus management tests
...
├── ISSUES.md               — Finding tracker (276 findings, 2 open)
