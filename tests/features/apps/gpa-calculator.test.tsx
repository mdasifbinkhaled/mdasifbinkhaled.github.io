...
describe('GpaCalculator', () => {
  it('renders default 3 courses with grade selectors', () => {
    ...
    const gradeSelects = screen.getAllByRole('combobox', { name: 'Grade' });
  ...
  it('updates GPA when grade is changed', async () => {
    const user = userEvent.setup();
    render(<GpaCalculator />);
    // Change first course from A to F via shadcn Select
    const gradeSelects = screen.getAllByRole('combobox', { name: 'Grade' });
    await user.click(gradeSelects[0]);
    await waitFor(() => {
      expect(screen.getByRole('listbox')).toBeInTheDocument();
    });
    const fOption = screen.getByRole('option', { name: /^F / });
    await user.click(fOption);
  ...
  it('adds and removes courses', () => {
    render(<GpaCalculator />);
    expect(screen.getAllByRole('combobox', { name: 'Grade' })).toHaveLength(3);

    fireEvent.click(screen.getByText(/Add Course/));
    expect(screen.getAllByRole('combobox', { name: 'Grade' })).toHaveLength(4);
  ...
  it('renders all 11 grade options when select is opened', async () => {
    const user = userEvent.setup();
    render(<GpaCalculator />);
    const gradeSelects = screen.getAllByRole('combobox', { name: 'Grade' });
    await user.click(gradeSelects[0]);
    ...
    expect(options[10]).toHaveTextContent('F');
  });
});
