...
export function NavigationSection({
  ...
  return (
    <div className={cn('flex-none', isCollapsed ? 'p-2' : 'px-6 pt-6 pb-2')}>
      ...
      <nav className="space-y-1">{mainNavItems.map(renderItem)}</nav>

      {isCollapsed ? (
        <div className="my-2 border-t border-sidebar-border/50" />
      ) : (
        <h4 className="text-xs uppercase font-semibold text-sidebar-foreground/80 mt-5 mb-3 tracking-wider sidebar-section-title">
          More
        </h4>
      )}
      <nav className="space-y-1">{secondaryNavItems.map(renderItem)}</nav>
    </div>
  );
}
