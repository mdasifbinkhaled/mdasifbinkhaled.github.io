...
export function Breadcrumbs() {
  ...
  return (
    <nav aria-label="Breadcrumb" className="flex items-center space-x-2 text-sm mb-6 px-4 py-2 bg-muted/50 rounded-lg">
        ...
        return (
          <div key={segment} className="flex items-center space-x-2">
            ...
            {isLast ? (
              ...
              <Link href={href} className="flex items-center gap-1 hover:text-primary transition-colors">
                {Icon && <Icon className="w-4 h-4" />}
                <span>{title}</span>
              </Link>
            )}
