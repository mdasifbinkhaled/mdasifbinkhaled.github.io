  ...
  useEffect(() => {
    const personSchema = {
      ...
      sameAs: [
        ...
        ...(siteConfig.links.twitter !== 'https://twitter.com/yourusername'
          ? [siteConfig.links.twitter]
          : []), // Add Twitter if not default
      ],
      alumniOf: [
        {
...
export function PublicationStructuredData() {
  useEffect(() => {
    const publicationsSchema = samplePublications.map(pub => ({
      ...
      ...(pub.venue && {
        isPartOf: { '@type': 'PublicationVolume', name: pub.venue },
      }), // Simplified, ideally more detailed
      ...
      publisher: {
        // Generic publisher, can be refined if each pub has specific publisher info
        '@type': 'Organization',
        name: pub.venue?.includes('IEEE')
          ? 'IEEE'
          : pub.venue?.includes('Springer')
            ? 'Springer'
            : 'Academic Publisher',
      },
    }));
    ...
    if (publicationsSchema.length > 0) {
      ...
      script.text = JSON.stringify(
        publicationsSchema.length === 1
          ? publicationsSchema[0]
          : publicationsSchema,
      );
      document.head.appendChild(script);
